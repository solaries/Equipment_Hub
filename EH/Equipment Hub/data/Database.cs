...
namespace Equipment_hub.Data.Models
{
    ...
    public partial class Equipment_hub_Equipment_Activity_Movement : Equipment_hub.Record<Equipment_hub_Equipment_Activity_Movement>
    {
        ...
        public static IEnumerable<Equipment_hub_Equipment_Activity_Movement> Query(Database db, string[] columns = null, long[] Id = null)
        {

            var sql = new Sql();
            if (columns != null) sql.Select(columns);
            sql.From("Equipment_hub_Equipment_Activity_Movement (NOLOCK)");
            if (Id != null)
                sql.Where("id IN (@0)", Id);

            return db.Query<Equipment_hub_Equipment_Activity_Movement>(sql);
        }
    }
