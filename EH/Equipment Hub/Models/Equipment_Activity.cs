...
{ 
    ...
    { 
        public string add_Equipment_Activity(Equipment_hub_Equipment_Activity new_Equipment_Activity, bool returnID = false ) 
         ...
         {
             var context = Equipment_hub.Data.Models.Equipment_hub.GetInstance();
             var actual = context.Fetch<Equipment_hub_Equipment_Activity_data>("select a.id , a.equipment_type , a1.Name_Of_Equipment  equipment_type_data  ,  a.Number_Of_Units , a.Rent_Rate ,    CONCAT( a2.Qunatity * a.Rent_Rate_Qunatity ,' ', b1.Duration_Name) Rent_Rate_data ,  b1.Duration_Name ,  a2.Price * a.Number_Of_Units * a.Rent_Rate_Qunatity Rent_Rate_Qunatity,a2.Price , a.Rent_Rate_Qunatity q,   a.Customer_Company ,  a3.Company_Name  Customer_Company_data  , a.Vendor_Company , a4.Company_Name  Vendor_Company_data  ,     a.Activity_Status , a5.Activity_Status_Name  Activity_Status_data  , a.activity_date    	 from Equipment_hub_Equipment_Activity a  inner JOIN    Equipment_hub_Equipment_Type a1 on a.equipment_type = a1.id  	  inner JOIN   	Equipment_hub_Rent_Rate a2 on a.Rent_Rate = a2.id   	  inner JOIN   	equipment_hub_duration_type b1 ON  a2.Duration_Type = b1.id  	  INNER JOIN  Equipment_hub_Customer_Company a3 on a.Customer_Company = a3.id   	  inner JOIN   Equipment_hub_Vendor_Company a4 on a.Vendor_Company = a4.id   	  inner JOIN   Equipment_hub_Activity_Status a5 on a.Activity_Status = a5.id " + sql);
             return actual;
         }  
